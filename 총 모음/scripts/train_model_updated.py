    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            if val_f1 > best_f1:
                ...
                print(f"   [OK]    (F1: {best_f1:.4f})")

        print(f"\n{'='*70}")
        print(f"[OK]  !")
        print(f"{'='*70}")
        print(f"[BEST] Best F1 Score: {best_f1:.4f}")
        print(f"⏰  : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"   : {output_dir}")
...
def main():
    ...
    #  (GPU  )
    config = {
        'data_path': 'data/processed/final_training_dataset.json',
        'output_dir': './models',
        'epochs': 5,  #    5 
        ...
        'warmup_steps': 500,
        'test_size': 0.2,
        'max_samples': 50000  #  : 50K  
    }

    print("\n" + "="*70)
    print("[TARGET] KoELECTRA     ")
    print("="*70)
    print("\n[INFO] :")
